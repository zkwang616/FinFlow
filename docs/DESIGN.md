...
- 审计副产物：任务结束自动生成 `trace.json`（每节点 I/O 摘要、耗时、action、错误），随报告输出
- 前端：任务创建页 + 实时看板（DAG 状态 + 事件日志）+ 报告预览页（**不含节点详情交互，前端此后搁置**）
...
- **定量分析引擎**：估值引擎（DCF / 可比公司估值，计算而非 LLM 编造）、敏感性分析
- **财务指标体系**：ROE、ROIC、负债率、流动比率、利息覆盖等核心比率
- **核心图表**：股价走势、财务趋势、同业对比（2-3 类）
- **补齐 8 个 agent**：新闻总结、竞争对手、催化剂、投资概览
- 新闻/情绪整合分析
- PDF 导出、FMP / yfinance 真实数据源（`mode=real`）
- 审计增强（副产物形式）：章节级溯源、事件回放文档
...
**项目核心是"分析股票"，不是可视化。** 可观测性/前端可视化降级为支撑与审计手段：

- 分析能力（数据 → 多角色 LLM 分析 → 报告）是产品主体，路线图以分析深度为优先
- 执行过程的可观测信息（节点 I/O 摘要、耗时、action、错误）以 **`trace.json` 审计文档**形式随报告输出，不再作为前端交互功能
- 前端控制台搁置：保留任务创建、DAG 状态展示、事件日志与报告预览；节点详情交互已移除
...
- **审计副产物（V1 已实现）**：任务结束由 `JobManager` 自动生成 `trace.json`（与报告同目录）——包含每个节点的 I/O 摘要、耗时、action、错误信息与全量事件，作为报告的可审计附件
- **事件回放**：`GET /api/jobs/{id}/events` 返回全量事件序列，可离线重放（V2 完善为文档化回放）
- **章节级溯源（V2）**：报告每个章节关联 `{数据源、数据日期、生成节点、agent 名、模型、耗时、状态}`
...
> **状态：V1 后搁置。** 前端仅保留任务创建、DAG 状态展示、事件日志与报告预览；节点详情交互（点击查看 I/O）已移除，审计信息改由 `trace.json` 文档输出。
...
- 右侧：事件日志流（滚动）+ 运行统计（总耗时、LLM 调用次数）
- 顶部：任务状态徽章

**页面 3：报告预览**

- iframe/HTML 渲染报告
- 下载/查看同目录 `trace.json`（审计副产物）
